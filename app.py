...
def score_resume(resume_text, jd_text):
    ...
    matched, missing = keyword_match(resume_text, kws)  # only check resume vs keywords
    ...
    # Weighted contribution (no extra /100 later)
    total_score += cat_score * weights[cat]

# Final normalized score (already on 0–100 scale)
